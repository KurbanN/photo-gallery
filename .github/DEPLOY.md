...
3. **Supabase → Authentication → Providers → Email:** включите Email, **отключите «Confirm email»** — вход сразу после регистрации, без писем.

4. **Supabase → Authentication → URL Configuration** (если когда-либо использовали magic link):
...
5. В `public/app-config.json` укажите `"appUrl": "https://kurbann.github.io/photo-gallery"` (для старых magic link, если остались).

6. На сервере API задайте `APP_PUBLIC_URL=https://kurbann.github.io/photo-gallery` и  
